...
int row=0;
        try {
            ...
            rs=st.executeQuery("select max(id) from employees");
            while(rs.next()){
            row = rs.getInt(1);}
...
public boolean isPhoneExist(String phone){
        try {
            ps = con.prepareStatement("select * from employees where phone =?");
...
String sql="insert into employees values(?,?,?,?,?,?)  ";
   ...
   //update user data
    public void update(int id, String username, String email, String pass, String phone, String address) {
        String sql = "update employees set username = ?,email = ?, password = ?, phone = ?,address = ?  where id = ?";
    ...
    public int getUserId(String email) {
        int id = 0;
        try {
            ps = con.prepareStatement("select id from employees where email =?");
            ps.setString(1, email);
            rs = ps.executeQuery();
            if (rs.next()) {
                id = rs.getInt(1);
